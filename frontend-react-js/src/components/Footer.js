...
const heading = 'eyebrow mb-3';
const link = 'block py-1.5 text-[15px] text-fg-soft hover:text-fg hover:no-underline';
...
export default function Footer({ signedIn = false, clearNav = false, className = '' }) {
  return (
    <footer className={`border-t border-ink-700 ${className}`}>
      <div className={`mx-auto w-full max-w-6xl px-5 pt-10 sm:px-8 lg:px-12 lg:pt-14 ${clearNav ? 'pb-28 sm:pb-8' : 'pb-8'}`}>
        <div className="grid gap-9 sm:grid-cols-2 lg:grid-cols-[1.7fr_1fr_1fr_1.3fr] lg:gap-14">
          <div>
            ...
            <p className="mt-2.5 max-w-xs text-sm leading-relaxed text-fg-mute">Know what you are lifting today, and beat what you lifted last time.</p>
          </div>
          ...
          <div className="card p-5 sm:col-span-2 lg:col-span-1 lg:border-0 lg:bg-transparent lg:p-0">
            <p className={heading}>Built by</p>
            <p className="font-display text-[22px] font-extrabold leading-tight">{DEVELOPER_NAME}</p>
            <p className="mb-4 mt-2 text-sm leading-relaxed text-fg-mute">Want to help build Blinkr? Get in touch.</p>
            <Link to="/support" className="btn-secondary border border-ink-500 bg-transparent hover:bg-ink-800">Contact {DEVELOPER_NAME}</Link>
          </div>
        </div>

        <div className="mt-10 flex items-center justify-between border-t border-ink-700 pt-5 text-[13px] text-fg-mute">
          <span>© {new Date().getFullYear()} Blinkr</span>
          <button type="button" onClick={backToTop} className="inline-flex h-11 items-center gap-1.5 rounded-lg px-2 hover:text-fg">
